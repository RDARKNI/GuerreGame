...
// Then in controller, the Role::host goes through history (except 0th element)
// and plays out each move This copies history
//*/
//
...
//       }
//     }
//     broadcast_inputs({{-1, -1}, ActionID::action, OptionID::option, {-1,
//     -1}});
//   } else {
...
//   can_act = role == Role::local || dh.get_curr_player_data().y ==
//   player_index;
// }
//
...
//   optionwin.show();
//   if (!curr_opts[opt_i].x) {
//     opt_i = 1 - opt_i;
//   }
//   inputs.opt = OptionID(curr_opts[opt_i].y);
...
//   size_t act_i = dh.get_next_action_index(inputs.act);  //
//   to_index(inputs.act); while (!curr_actions[act_i].x) {
//     ++act_i;
//   }
//   const size_t acount{curr_actions.size()};
//   auto mov{[this, &act_i, &curr_actions]() {
...
//         break;
//       case conf:
//         if (curr_actions[act_i].x) {
//           menu_stack.push(MenuState::opt_sel);
//           fprintf(stderr, "sourceGG: %d,%d\n", inputs.src.y, inputs.src.x);
//           fprintf(stderr, "dstGG: %d,%d\n", inputs.dst.y, inputs.dst.x);
//           inputs.act = ActionID(curr_actions[act_i].y);
//           return MenuAction::reg;
//         }
//         break;
//       case back:
...
//   targetwin.hide();
//   optionwin.show();
//   size_t opt_i = dh.get_next_option_index(inputs.opt);  //
//   to_index(inputs.opt); const auto& curr_options{dh.get_curr_options()}; auto
//   mov{[this, &opt_i, &curr_options]() { optionwin.move_cursor(opt_i); }};
//   mov();
...
//         break;
//       case conf:
//         if (curr_options[opt_i].x) {
//           inputs.opt = OptionID(curr_options[opt_i].y);
//           menu_stack.push(MenuState::dst_sel);
//           return MenuAction::reg;
//         }
//         break;
...
//     int sending_player = role == Role::host ? dh.get_curr_player_data().y :
//     0; size_t ret = receive_inputs(sending_player, received_input, 1); if
//     (ret == 0) {
//       nodelay(stdscr, true);
//       return input_handler();
//     } else if (ret < 0) {
...
//     }
//   }
//   can_act = role == Role::local || player_index ==
//   dh.get_curr_player_data().y; inputs = {}; return 0;
// }
