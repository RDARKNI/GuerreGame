 ...
 public:
  ...
  size_t get_next_action_index(ActionID act) const {
    ...
    for (size_t i{}; i < curr_actions.size(); ++i) {
      if (curr_actions[i].type == act && curr_actions[i].mods) {
        return i;
      }
    }
    return 0;
  ...
  size_t get_next_option_index(OptionID opt) const {
    ...
    for (size_t i{}; i < curr_options.size(); ++i) {
      if (curr_options[i].type == opt && curr_options[i].mods) {
        return i;
      }
    }
    return 0;
  ...
  bool validate_input(const UserInput& inputs) {
    if (inputs.act <= ActionID::action || inputs.act >= ActionID::COUNT) {
    ...
    if (actor.get_player_i() != curr_player) {
      return false;
    }
    if (!action.other_cond(actor)) {
      return false;
    }
