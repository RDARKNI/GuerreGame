...
class ToCastle final : public Action {
 public:
  ...
  void perform(Coord source, int opt, Coord target) const override {
    ...
    player.add_piece(std::make_unique<Pion>(player, CHATEAU, target));
    player.change_gold(-piece_constants[CHATEAU].cout);
  }
  bool get_fatigue_condition(Coord source) const override {
    const Pion& actor = *(*board)[source];
    return !actor.get_fatigue() &&
           actor.get_player().get_gold() >= piece_constants[CHATEAU].cout;
  }
};
