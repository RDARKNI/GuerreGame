 ...
 public:
  void display() {
    if (*data) {
      ...
      mvwprintw(win, 2, 0, strings[1], data->get_fat());
 ...
 public:
  void display() {
    ...
    for (size_t i = 0; i + offs < strings.size(); ++i) {
      wattrset(win, i + offs == sely ? A_STANDOUT : 0);
...
class ActionWindow : public SubWindow<std::vector<ActionData>> {
 public:
  using SubWindow::SubWindow;
  void move_cursor(size_t new_sely) {
    if (sely < data->size()) {
      wattrset(win, !(*data)[sely].mods ? A_DIM : 0);
      mvwaddstr(win, sely, 0, strings[index((*data)[sely].type)]);
    }
    sely = new_sely;
    wattrset(win, int((!(*data)[sely].mods ? A_DIM : A_NORMAL) | A_STANDOUT));
    mvwaddstr(win, sely, 0, strings[index((*data)[sely].type)]);
    wattrset(win, A_NORMAL);
    wrefresh(win);
  }
 ...
 public:
  void display() {
    for (size_t i = 0; i < data->size(); ++i) {
      wattrset(win, !(*data)[i].mods ? A_DIM : 0);
